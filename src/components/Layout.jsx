...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-primary border-t border-white/10 flex items-center justify-around h-16 px-1">
        {items.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center gap-1 flex-1 h-full transition-colors ${
                active ? 'text-accent' : 'text-primary-foreground/55'
              }`}
            >
